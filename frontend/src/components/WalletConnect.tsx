...
export function WalletConnect() {
  ...
  const executeConnect = async (preferredWallet?: 'xverse' | 'leather') => {
    ...
    try {
      // Clear any stale provider ID in case another wallet was previously selected
      try {
        clearSelectedProviderId();
      } catch {}
      ...
      // Don't show error if user simply cancelled the modal
  ...
  return (
    <div className="flex items-center gap-1.5 sm:gap-2 min-w-0">
      {/* Live STX Balance */}
      <div className="hidden sm:flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 bg-slate-100 border border-slate-200 rounded-lg text-xs font-mono text-slate-700 whitespace-nowrap">
      ...
      {/* Truncated Address */}
      <div className="flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-xs font-mono text-slate-800 shadow-sm whitespace-nowrap">
      ...
      <button
        ...
        className="px-2 py-1.5 text-xs text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors border border-transparent hover:border-red-200 shrink-0"
      >
        ✕
      </button>
    </div>
